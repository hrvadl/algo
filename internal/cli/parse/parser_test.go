...
func TestEqualityFromString(t *testing.T) {
	tc := []struct {
		name     string
		str      string
		expected []float64
	...
	for _, tt := range tc {
		t.Run(fmt.Sprintf("%s: %s", tt.name, tt.str), func(t *testing.T) {
			t.Parallel()
			actual, _ := EquationFromString(tt.str)
			if !slices.Equal(tt.expected, actual) {
				t.Fatalf("expected: %v\ngot:%v", tt.expected, actual)
			}
		})
...
func TestEqualityOrInequalityFromString(t *testing.T) {
	tc := []struct {
		name     string
		str      string
		expected []float64
	...
	for _, tt := range tc {
		t.Run(fmt.Sprintf("%s: %s", tt.name, tt.str), func(t *testing.T) {
			t.Parallel()
			actual, _, _ := EquationOrInequationFromString(tt.str)
			if !slices.Equal(tt.expected, actual) {
				t.Fatalf("expected: %v\ngot:%v", tt.expected, actual)
			}
		})
